 ...
 */
function CameraPreview({ ref, active, compact = false, className }: CameraPreviewProps) {
    ...
    return (
        <div
            className={cn(
                ...
                compact ? 'h-48 w-full max-w-xs' : 'aspect-video w-full max-w-md',
                className,
            )}
        >
            <video
            ...
            {!active && (
                <div className="flex h-full w-full flex-col items-center justify-center gap-2 text-muted-foreground">
                    ...
                    <span className="text-xs">{t('vision.camera.inactive')}</span>
                </div>
            )}
